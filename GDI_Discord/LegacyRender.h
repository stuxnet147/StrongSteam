...
private:
	...
	FColor ColorBlend(FColor prev, FColor cur)
	{
		...
		mix[0] = round((added[0] * added[3] / mix[3]) + (base[0] * base[3] * (1 - added[3]) / mix[3])); // red
		mix[1] = round((added[1] * added[3] / mix[3]) + (base[1] * base[3] * (1 - added[3]) / mix[3])); // green
		mix[2] = round((added[2] * added[3] / mix[3]) + (base[2] * base[3] * (1 - added[3]) / mix[3])); // blue
...
public:
	...
	void FillRectangle(const Vector2& Start, const Vector2& Size, const FColor& Color) {
		for (int i = 0; i < Size.y + 1; i++) {
			...
			__stosd((PDWORD)&Arr[off], Color.Get(), Size.x + 1);
		}
	}
